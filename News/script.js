...
export async function renderNewsSection(containerSelectorOrElement, cardStyle, typeFilter, limit = 3) {
  let container;

  if (typeof containerSelectorOrElement === 'string') {
    // Se começa com . ou #, use querySelector (classe ou id)
    if (containerSelectorOrElement.startsWith('.') || containerSelectorOrElement.startsWith('#')) {
      container = document.querySelector(containerSelectorOrElement);
    } else {
      // Se não, tente por ID
      container = document.getElementById(containerSelectorOrElement) || document.querySelector(containerSelectorOrElement);
    }
  } else {
...
function createCard(item, cardStyle) {
  ...
  else if (cardStyle === CardSize.INSTITUTIONAL) {
    card.classList.add('news__item', 'news__item-link');
    card.innerHTML = `
      <img
        class="news__item-img"
        src="${item.image}"
        alt="Imagem relacionada à ${item.title}"
      />
      <div class="news__item-content">
        <h4 class="news__item-h4">${item.title}</h4>
        <p class="news__item-p">${parseDate(item.date)}</p>
      </div>
  `;
  }
  card.onclick = () => window.location.href = `/News/new_details.html?id=${item.id}`;
